...
These are the project's own, written by it and never upgraded — distinct from `conventions/`, Catalyst's always-applied set that arrives with the bundle. They live as annexes owned and indexed by `architecture.md`, lazy-loaded like folder documents, with their load triggers listed in the entry document. A convention change updates the annex, not the index. On conflict: feature contracts and `architecture.md` win over annexes; annexes win over generic tool or skill guidance. Annexes never carry workflow rules or behavior contracts.

...
// ! Three sources, and missing any one leaves a border stale: the container resizing, the CONTENT resizing inside a container that never changed size (the common case in a dialog), and scroll itself, bound passively on the element above.
...
.scroll-area {
   ...
   * rule — the layout shift `layout-stability.md` forbids.
   *
...
.edge-left {
  border-left-color: rgb(var(--v-theme-scroll-edge));
}

.edge-right {
  border-right-color: rgb(var(--v-theme-scroll-edge));
}
</style>

...
**Page canvas** — the colour the page itself is painted in, behind everything. Distinct from a **surface**: the ground a card, table, modal, input or menu sits on, which rises off the canvas rather than matching it. The two are separate colours in both faces, and a component asking for "the background" means the surface, never the canvas. Nuxt UI names only the surface, so the canvas is the project's own token — the distinction is easy to lose and repainting the surface to the canvas's value silently flattens every raised element in the app.

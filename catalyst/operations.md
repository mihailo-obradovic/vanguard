...
Frontend mutation audit (ADR 009; `stryker.config.json`, same cadence and triage rule as the backend's). Scope is the data layer — `web/utils`, `web/services`, `web/stores`, `web/composables` — plus `web/plugins` and `web/regle-config.ts`, which joined once they had tests. Components, pages, layouts, and middleware are out because only two components are tested; every mutant in the rest would report as noise rather than signal. `.vue` files rejoin the same way the plugins did — when enough of them are tested that the report reads as signal. The untested composables stay in scope on purpose: Stryker separates `NoCoverage` from `Survived` and prints a covered-code score beside the total, which is the honest analogue of the backend's `--covered-only` without a hand-maintained exclusion list that rots.
...
Second audit on `variant/vuetify` (2026-08-13), after the app-shell work below brought `web/plugins` and `web/regle-config.ts` into scope: **92.98%** total, **97.11%** on covered code (470 mutants, 13 surviving, 20 uncovered). The total rose and the covered score dipped by the same event — new files arrived carrying three accepted survivors of their own. The audit paid for itself on `regle-config.ts`: the bare `email` and `sameAs` messages had no assertion anywhere, so either entry could be deleted and every form would silently fall back to the library's hardcoded English. Newly accepted survivors, on top of the categories above:
...
### The app shell and plugins — what is tested and what is not

Recorded because the alternative is invisible: a file at 0% looks the same whether the omission was deliberate or forgotten. The rule applied is `stacks/frontend/nuxt/testing.md` — logic gets a unit test, presentation is proven by the live browser walk (`decisions/008`).
...
| `app.vue`                                       | 87.5% | **Tested.** The `isLoggedIn` watcher is the only redirect path for a session change with no navigation — login and logout are dialogs, so `middleware/auth.global.ts` never runs for them. The shell around it is markup. |
...
| `components/_shared/TheFooter.vue`              | 0%    | **No test, deliberately.** Presentational; `testing.md` names it as the counter-example. Live browser walk.                                                                                                               |
| `error.vue`                                     | 7%    | **No test, deliberately.** A status-code ternary and a `JSON.stringify`; a test would pin i18n key wiring, not behavior. Live browser walk.                                                                               |
...
| `plugins/hide-devtools-webcomponents.client.ts` | 12.5% | **No test, deliberately.** Dev-only DOM housekeeping that never runs in production.                                                                                                                                       |

! The numbers this task started from — `i18n/i18n.config.ts` at 12.5% and `plugins/vuetify.ts` missing from the report entirely — were a **stale local `coverage/` directory**, not a measurement artifact and not a gap. `coverage/` is gitignored, so a leftover report from before those specs were written outlives them and reads as a coverage hole. Both measure 100% on a fresh run. Re-run `pnpm test:coverage` before believing any number in there.

Coverage after this work: **33.64% statements / 36.21% lines** overall (from 30.15% / 32.73%). `web/plugins` as a directory sits at 57.89% lines, held down by the two files deliberately left untested above.

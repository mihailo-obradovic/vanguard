...
Frontend mutation audit (ADR 009; `stryker.config.json`, same cadence and triage rule as the backend's). Scope is the data layer — `web/utils`, `web/services`, `web/stores`, `web/composables` — plus `web/plugins`, `web/regle-config.ts`, and `web/i18n/i18n.config.ts`, which joined once they had tests, and `web/components`, `web/layouts` and `web/app.vue`, which joined once the shell and the two logic-bearing components were tested. Pages and middleware stay out: pages are on the live browser walk (`decisions/008`), so every mutant there would report as noise rather than signal. The untested composables stay in scope on purpose: Stryker separates `NoCoverage` from `Survived` and prints a covered-code score beside the total, which is the honest analogue of the backend's `--covered-only` without a hand-maintained exclusion list that rots.

Two things about `.vue` in particular:

- **Stryker mutates only the `<script>` block.** Template logic is never mutated, which is why four of this branch's components yield no mutants at all: `SkipLink` has no script, `Default.vue`'s is three lines of composable calls, `TheFooter` derives one constant, and `FieldErrors`' whole behaviour — `errors[0] ?? ''` — lives in its template behind a type-only `defineProps`. Read a `.vue` score as a statement about its script, not about the component.
- **Compiler macros would need excluding from instrumentation — but not here.** Stryker declares its coverage helpers locally in the script block, and `withDefaults(defineProps<…>(), { … })` and `defineModel(…, { … })` are hoisted out of `setup()`; referencing the helpers from there is a `@vue/compiler-sfc` compile error that fails the whole dry run. This branch uses neither form, so no component needs a `// Stryker disable all` pair. `variant/vuetify` needs one in fourteen components — do not drop that when merging in either direction.
...
Third audit (2026-08-15), the first with `.vue` in scope: **92.76%** total, **95.60%** on covered code (539 mutants, 23 surviving, 16 uncovered, plus 24 ignored as static). Adding components, layouts and `app.vue` moved the score by less than two points, because this branch's component surface is small and what exists is either tested or scriptless — the whole `.vue` contribution is 20 mutants across three files. It found no new gaps. Newly accepted survivors:
...
`variant/vuetify` runs the same scope and scores markedly lower (81.17% / 87.64%) — not a regression, but the first measurement of a much larger component surface. Do not read the two numbers as comparable.
...
| `components/_shared/LocaleSwitcher.vue`         | 100%  | **Tested.** Its setter writes through `setLocale` specifically to persist the choice; asserting the rendered language would not catch losing it.                                                                                                |
| `layouts/Default.vue`                           | 100%  | **Tested.** Decides what a guest, a user and an admin each see, and owns logout. The admin links are a convenience, not the control — the API authorizes those pages itself — but showing them advertises a door the user cannot open.          |
...
| `error.vue`                                     | 0%    | **No test, deliberately.** Eight lines that render the error object into the layout. Live browser walk.                                                                                                                                         |
| `components/_shared/FieldErrors.vue`            | 0%    | **No test, deliberately.** Renders the first message of a prop array; no logic of its own. Contributes no mutants either — its script is a type-only `defineProps`, and the `?? ''` fallback lives in the template.                             |
| `components/_shared/SkipLink.vue`               | n/a   | **No test, deliberately.** No script block at all: an anchor whose entire behaviour is a CSS `:focus` rule moving it on screen. The keyboard path is an accessibility case for the live browser walk.                                           |
...
Coverage after this work: **54.79% statements / 54.30% lines** overall, up from 43.52% / 43.71%. What remains uncovered is almost entirely `pages/`.

...
**Molded** code has one primary function per component or module, names that carry the explanation, and comments and exceptions that are rare and warranted. Agent-generated code drifts verbose — narration comments, one-use helpers, defensive wrapping. This document defines the molded standard and the `/mold` procedure that reshapes code back to it.
...
- the prefilled dev credentials on the login page — intentional

---

## The /mold run

1. **Target.** The paths passed as arguments; with none, the files changed since the branch point (`git diff --name-only $(git merge-base HEAD master)`) plus uncommitted changes. List the targets before touching anything.
2. **Dispatch.** One `code-simplifier` agent for the whole target set — a single agent with the cross-file view split decisions need, never a per-file fan-out. Its instructions: read this document and `conventions/code-annotations.md`, apply the within-file tier directly, and _return_ (not apply) the file-graph proposals and the flag list.
...
4. **Verify.** For touched frontend files: `pnpm lint`, `pnpm typecheck`, and the vitest files covering the touched files. For touched backend files: `vendor/bin/pint --test` and the Pest tests covering them. A failure is fixed or its causing change reverted before the run reports done.

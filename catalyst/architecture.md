...
| Universal Rules           | every project, any stack                   | prime directive version bump, user-approved — rare |
| Stack Modules (`stacks/`) | every project, one module per layer it has | per-project decision record                        |

**Agents never change this file on their own initiative.** An agent may propose a change and draft the decision record (or the prime directive change); the user decides. This holds at every level.

Stack modules name tools and may add their own rules; they never restate a Universal Rule — and their named tools are themselves defaults, swappable by decision record under the same Universal Rules. An optional layer (a module not in the default set) is adopted when its trigger fires — not in anticipation — and never replaces the default stack wholesale.
...
| `tailwindcss`, `@tailwindcss/vite` | frontend       | `variant/shadcn-vue`: the styling system shadcn-vue's vendored components are written against. `@tailwindcss/vite` is v4's Vite plugin, wired in `nuxt.config.ts`; the CSS entry (`web/assets/styles/main.css`) imports `tailwindcss` and carries the whole token set — the headless module's tokens sat beside it through the conversion and were deleted in Phase 4 part 8, once nothing read them.                                             | user, 2026-09-09 |
| `shadcn-vue`                       | frontend (dev) | `variant/shadcn-vue`: the component CLI (`shadcn-vue init` / `add`), run through `pnpm dlx`.                                                                                                                                                                                                                                                                                                                                                      | user, 2026-09-09 |
| `shadcn-nuxt`                      | frontend       | `variant/shadcn-vue`: required by the official Nuxt install guide — prevents console warnings from Nuxt's auto-import and configures the component directory/prefix.                                                                                                                                                                                                                                                                              | user, 2026-09-09 |
| `reka-ui`                          | frontend       | `variant/shadcn-vue`: the headless primitive layer shadcn-vue's components are built on — the Vue equivalent of Radix, and the only component-library option `shadcn-vue init` offers (confirmed live: the prompt listed exactly one choice). Installed by `init`/`add` as components need it, not hand-pinned.                                                                                                                                   | user, 2026-09-09 |
| `class-variance-authority`         | frontend       | `variant/shadcn-vue`: the variant API every vendored shadcn-vue component uses.                                                                                                                                                                                                                                                                                                                                                                   | user, 2026-09-09 |
| `clsx`, `tailwind-merge`           | frontend       | `variant/shadcn-vue`: `web/lib/utils.ts`'s `cn()` helper, which every vendored component calls to merge classes.                                                                                                                                                                                                                                                                                                                                  | user, 2026-09-09 |
| `@lucide/vue`                      | frontend       | `variant/shadcn-vue`: the icon set, chosen to match what shadcn-vue's own components import by default — anything else means patching every vendored component and fighting `shadcn-vue diff` (Phase 6's periodic check). Answers the project's open "which icon library" question for this variant specifically.                                                                                                                                 | user, 2026-09-09 |
| `tw-animate-css`                   | frontend       | `variant/shadcn-vue`: installed by `shadcn-vue init` as a default dependency (its official manual-install guide lists it alongside the other four in one command) — included per the same rule as the rest rather than deferred.                                                                                                                                                                                                                  | user, 2026-09-09 |
| `vue-sonner`                       | frontend       | `variant/shadcn-vue`: the toast library behind the `$toast` seam, installed by `shadcn-vue add sonner` and styled through the vendored `ui/sonner` wrapper so toasts take the theme's variables. Replaces `vue-toastification`, whose Vue 3 line (`2.0.0-rc.5`) was published 2021-10-18, never promoted off npm's `next` tag, and has had no release since. Master and `variant/vuetify` still carry the old one — their swap is its own ticket. | user, 2026-09-10 |
| `@nuxtjs/color-mode`               | frontend (dev) | `variant/shadcn-vue`: switches between `main.css`'s light and `.dark` faces — the mechanism shadcn-vue's Nuxt dark-mode guide prescribes. Its synchronous head script stamps `dark` on `<html>` before first paint, which a hand-rolled toggle would have to reimplement to avoid a light flash. `variant/nuxtui` already runs it, through `@nuxt/ui`.                                                                                            | user, 2026-09-13 |
...
- Dashboards and alert rules are versioned files in the repository, provisioned on startup — never hand-built in a monitoring UI; a dashboard that dies with a volume reset was never finished. Identifiers the provisioned files depend on (datasource UIDs) are pinned. At minimum, a service-down alert exists.
...
The stack is a **selection** — one module per layer from the index below, each living in `stacks/`. A module names the concrete tools and binds the Universal Rules to them; it never restates a Universal Rule, and its tools are defaults, swappable per project by decision record. The default set (the whole Default column) is what a project starts with; `project-summary.md` (Technical Stack) records what it actually runs — every layer's module, any deviation, and any optional layer it adopts. Each module's document is normative — read the ones the project adopted.
...
Maintenance is an **optional layer** as well: adopted once the project has a committed lockfile or pinned image tags to keep current, it names the bot that moves those pins and the review that gates them (`stacks/maintenance/renovate.md`). It loosens nothing above it — pins stay exact, and bumping a dependency the project already has is not a way around the Dependency Change Rule, which still owns every _new_ one.

...
The locked invariant is in the Flow Index (`prime-directive.md`). No feature document — `architecture.md` is the contract; the reviewed plan is a decision record (type `bootstrap`) so it stays in the repo. Weight `Medium` or `Hard`, never `Minor`.

## Steps

1. Confirm the weight (`Medium`, or `Hard` for several services at once).
2. Plan first; the user approves the plan before scaffolding.

...
- **Dracula on the seven semantic aliases.** Custom `@theme static` colours in `main.css`. Each accent carries two shades: 400, Dracula's own pastel, for the dark face, and for the light face the darkest step of that hue still clearing 4.5:1 under white — primary 700 `#8144c5` (5.89:1), secondary 700 `#bf0086` (5.90), success 800 `#007d2f` (5.28), info 800 `#007687` (5.33), warning 800 `#8c5400` (6.21), error 600 `#db0026` (5.20). One shared shade is not available: all six at 700 drops success to 3.69, info to 3.74, warning to 4.46. The `dracula` neutral ramp keeps all eleven — it is the only ramp the bg, text and border tokens consume, and `neutral` would shadow Tailwind's own palette.
- **The other nine steps of each accent ramp are gone** (66 lines of `@theme` for 12). Nuxt UI's components never name an accent shade — they read `--ui-<accent>` and alpha-derive every variant — and neither the library nor this app uses one. Consequence: `--ui-color-<accent>-<shade>` is still declared for the dropped steps and now resolves empty, so a stray `bg-primary-100` does nothing rather than painting wrong.
- **The dark block's accents are load-bearing**, though they match Nuxt UI's defaults: the light block matches `<html>` in both faces and is declared after Nuxt UI's `.dark`, so deleting them paints dark mode in the light face's darkened accents. Measured by deletion, as was the retirement of the old `:root.dark` selector — a bare `.dark` wins on source order, and the warning claiming it could not was wrong.

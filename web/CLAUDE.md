...
- `composables/` — `useAppQuery` / `useAppMutation` (the only query/mutation wrappers components may use), `useValidationErrors` / `useExternalErrors` (server-422-to-Regle bridge), `useDialogForm` (a dialog form's cancel/confirm/reset trio, and where the reset-timing rule is decided), `useConfirmOnEnter` (the Enter-confirms handler, and where the list of elements that keep Enter to themselves lives), `useCookieConsent`.

...
- Route decisions (`authRedirectLogic`, default-deny): `/` → `/home` always; guests on any page not in the guest-only/shared lists → `/home` (there is no dedicated guest-only auth route to redirect to — login/register/forgot-password are dialogs opened from `web/layouts/Default.vue`); logged-in users on the guest-only `/password-reset/*` page → `/home`. No return-URL preservation.
...
- SPA: `web/plugins/auth-loader.ts`, `web/middleware/auth.global.ts` + `web/utils/authRedirectLogic.ts`, `web/utils/{fetcher,handleApiError}.ts`, `web/stores/useAuthStore.ts`, `web/services/auth.api.ts` + `web/services/queries/useAuthQueries.ts`, the layout's `LoginDialog`/`RegisterDialog` (`web/components/users/`), opened from `web/layouts/Default.vue`.
...
- Frontend: `web/utils/_tests/authRedirectLogic.spec.ts` (guest redirects to home, reset-prefix match, authed users away from the guest-only password-reset page, default-deny, root alias); `web/components/users/_tests/{LoginDialog,RegisterDialog,ForgotPasswordDialog}.spec.ts`; `web/components/_shared/_tests/FormDialog.spec.ts`; `web/layouts/_tests/Default.spec.ts` (opening each dialog from the nav, the hand-off between them); `web/services/_tests/auth.api.spec.ts` (every session endpoint, including the web-route paths the stateful posture depends on); `web/services/queries/_tests/useAuthQueries.spec.ts` (the store side effects of login, register, refresh, logout); `web/stores/_tests/useAuthStore.spec.ts`; `web/utils/_tests/fetcher.spec.ts` (the client half of the 419 recovery); `web/plugins/_tests/auth-loader.spec.ts` (the CSRF cookie awaited before the session read, the user restored from the cookie, an expired session and a schema-violating user each leaving nobody signed in, and an unprimeable CSRF cookie failing the boot).
...
`php artisan test` is green against the MySQL `vanguard_testing` database (`operations.md`) and `route:cache` succeeds; the frontend suite covers the redirect, store, fetcher and boot-plugin cases above. The endpoint table, config values and redirect rules are traced line-by-line to source. A live login with `Test@Example.com` confirms mixed-case sign-in, which is why the read path carries no `lowercase` rule.

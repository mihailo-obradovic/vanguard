...
**Adds:** a versioned `renovate.json` at the repository root, plus the bot's platform enablement. Nothing at runtime — no library, no service, no import; an update bot is repository infrastructure, never part of the application image.
...
- It moves existing pins; it never introduces a dependency. The Dependency Change Rule is untouched: a new runtime dependency, framework, build plugin, or test tool still needs the user's explicit approval and an `architecture.md` update in the same change. A Renovate PR that would add a package is closed until that gate is passed, not merged because a bot opened it.
- It serves the pinning rule rather than excusing it: lockfiles and image tags stay exact, and the pin moves in a reviewed commit instead of a floating range moving it silently. Ranges are never widened to make the bot quieter, and stated minimums still govern — an update that crosses a minimum, drops a supported runtime, or swaps a named tool belongs to the decision record that owns that choice.

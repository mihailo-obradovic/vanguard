...
- **Component** — `@testing-library/vue` preferred (its queries are behavior-oriented by construction) over raw `@vue/test-utils` assertions; `renderSuspended` from `@nuxt/test-utils` is the Testing Library form of `mountSuspended`, for a component that needs the Nuxt runtime. Used sparingly, per the rule above. Three environment facts bite every such spec, all worked in `components/_shared/_tests/CookieConsentBanner.spec.ts`: `renderSuspended` supplies the Nuxt runtime but **not** the Vue plugins Nuxt plugins register, so Vuetify has to be passed in `global.plugins` or every component dies on a missing defaults instance; an overlay teleports outside the container Testing Library's `cleanup` owns, so the previous test's markup survives into the next one unless `document.body` is cleared too; and happy-dom has no `visualViewport`, which Vuetify's overlay positioning reaches for as a bare global — optional chaining does not save an undeclared name from a `ReferenceError`, so it must be stubbed rather than guarded.

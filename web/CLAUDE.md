...
- A form field is a `<u-form-field>` wrapping a `<u-input>`, never hand-written `<label>`/`<input>`/error markup: the component owns the generated `id`, the `for` pairing, the `aria-invalid`/`aria-describedby` wiring, and — through `config/nuxt-ui/form-field.ts` — the reserved error line that stops a message shifting every field below it. The one exception is the role `<select>`, which stays native and pairs its label by hand; the reason is recorded at both call sites.

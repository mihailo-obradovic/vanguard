...
- `components/_shared/` — auto-imported shared components (`components.dirs` in `nuxt.config.ts`); everything else is explicitly imported. `UIField.vue` and `UIDialog.vue` (+ `UIDialogActions.vue`) remain the primitives `UserFormDialog`/`UserGqlFormDialog` build from, pending their own Phase 4 conversion; `FormDialog.vue` is the shadcn-vue equivalent the auth dialogs use (`web/components/ui/dialog`, `useConfirmOnEnter`, `UIScrollArea` for the scrollable body) — two dialog primitives coexist on this branch until Phase 4 finishes (`catalyst/decisions/014_ui_shadcn-dialog-auth-composition.md`). `AuthCard.vue` is the shell the remaining standalone auth screen (`password-reset`) is made of. `RoleBadge.vue` is the users table's status pill; `VerificationBadge.vue` is shared between the table and `UserCard.vue`'s inline verification row.
- `components/<resource>/` — feature components, imported explicitly: `users/UserFormDialog.vue`, `users/UserGqlFormDialog.vue` own the forms their pages used to hold inline; `users/{Login,Register,ForgotPassword}Dialog.vue` are opened from `web/layouts/Default.vue`, not from a page; `users/UserCard.vue` is `pages/profile.vue`'s entire content — a true inline edit, not a dialog (`catalyst/decisions/014_ui_shadcn-dialog-auth-composition.md`).
...
- A form field is a `<UIField>`, never hand-written `<label>`/`<input>`/error markup: the component owns the generated `id`, the `for` pairing, the `aria-invalid`/`aria-describedby` wiring and the reserved error line — two lines' worth, derived from the line-height rather than a pixel constant, because a server 422 wrapping inside a narrow card would otherwise move every field below it. Native attributes (`type`, `required`, `:disabled`) fall through to the control; a control that is not an `<input>` — the role `<select>` — comes in through the slot and takes the same `controlId`.
- A dialog is a `<UIDialog>`: it owns the overlay, `role="dialog"`/`aria-modal`, the title it is labelled by, initial focus, the Tab trap and focus restoration on close, and asks to close through `@close` rather than mutating anything itself. It also bounds its own height and scrolls its body: the overlay is fixed and centred and `main.css` pins the page to `overflow-y: hidden`, so an unbounded panel taller than the viewport puts its submit button somewhere no scroll can reach. The body is a `<UIScrollArea>`, which marks whichever edge is currently hiding content. Restoration survives the opener being deleted: when the control the dialog was opened from is no longer in the document — a deleted row, above all — focus goes to the `#main-content` landmark instead of silently falling to `<body>`. That id is `layouts/Default.vue`'s to publish and `SkipLink.vue`'s other consumer; it carries `tabindex="-1"` for both of them, and a dialog rendered outside that layout simply leaves focus where the browser put it. Pages hold no template refs and no focus watchers. The buttons row goes in `<UIDialogActions>` inside the dialog's own content — a form dialog's submit button has to stay inside its `<form>`.

...
describe('ReservedLabel', () => {
  ...
  it('shows the active variant', async () => {
    ...
    expect(container.querySelector('.reserved-label-shown')?.textContent).toBe(
      'Delete'
    );
  });
  ...
  it('keeps every other variant in the box so the widest one reserves it', async () => {
    ...
    const ghosts = [...container.querySelectorAll('.reserved-label-ghost')];
  ...
  it('reserves the same set once the state has changed', async () => {
    ...
    expect(container.querySelectorAll('.reserved-label-ghost')).toHaveLength(2);
  });

  it('hides the ghosts from assistive technology', async () => {
    const { container } = await render('idle');

    for (const ghost of container.querySelectorAll('.reserved-label-ghost')) {

...
- A project owns its context and its copy of the rules; the copied `architecture.md` is the project's own from spawn onward and evolves through its decision records.
...
- Commit the draft document (feature, decision record, or experiment) and its `project-summary.md` row on the `master` branch before branching — this reserves the number and keeps the index authoritative. The approval flip (`Draft` → `Approved`, `Proposed` → `Accepted`, an experiment's `Proposed` → `Running`) lands there too, before the branch exists.

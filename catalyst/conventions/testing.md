...
3. **Mutation testing as a periodic audit** — the automated version of tier 2: mutate the code, rerun the suite, and every _surviving mutant_ is a change no test noticed. Backend: Pest's built-in mutation testing (`pest --mutate`); frontend: Stryker over the data layer. Run per the operations runbook, triage every survivor (fix the test or record why the mutant is acceptable), no CI gate — like coverage, it is a measurement, not a target.

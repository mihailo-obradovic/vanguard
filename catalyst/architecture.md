...
| `vue-sonner`        | frontend | The toast library behind the `$toast` seam (`web/utils/toast.ts`), mounted once in `web/app.vue` with its colours pointed at the theme tokens. Replaces `vue-toastification`, whose Vue 3 line (`2.0.0-rc.5`) was published 2021-10-18, never promoted off npm's `next` tag, and has had no release since. The same library every variant uses, so the seam's file is identical across branches. | user, 2026-09-13 |

...
Standing decisions like "negative values are signal, never clipped", "walk-forward evaluation only". Distinct from decision records (architectural *why*, lazy-loaded) and Protected Areas (load-bearing *contracts*): a domain decision is a locked *stance*.

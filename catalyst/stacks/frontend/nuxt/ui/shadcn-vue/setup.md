...
**Two faces, one of them not yet reachable.** Both blocks carry real values, but nothing applies the `dark` class — the app renders light-only until a colour-mode mechanism lands. Verify the dark face by stamping `dark` on `<html>` by hand.

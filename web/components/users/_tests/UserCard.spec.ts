...
const CURRENT_PASSWORD = 'Current password (required when changing password)';
const NEW_PASSWORD = 'New password (leave empty to keep current)';
...
async function fillInANewPassword() {
  ...
  await fireEvent.update(field(NEW_PASSWORD), 'hunter2hunter2');
  await fireEvent.update(field(CONFIRM_NEW_PASSWORD), 'hunter2hunter2');
}
...
describe('UserCard', () => {
  ...
  it('offers the password fields once editing starts', async () => {
    ...
    expect(field(CURRENT_PASSWORD)).toBeTruthy();
    expect(field(NEW_PASSWORD)).toBeTruthy();
  });

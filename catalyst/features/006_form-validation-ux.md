...
- `web/composables/useDialogForm.ts` — a dialog form's cancel / confirm / reset trio, and the single place the reset timing is decided.
...
- Known gaps (recorded): `name` and `current_password` still declare bare rules and show the generic `validation.*` copy — only the fields the factories own (the email and the password pair) reach `labeledRules`. Closing that means each form passing its `common.fields.*` label, deferred with the rest of the component work.

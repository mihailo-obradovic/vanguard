...
Standing risk: no live browser walk on this branch. The field-named localized messages were walked on `master`'s UI, which shares the helpers but not the forms, so the component specs stand in for them here — including for the two behavior changes the factory wiring accepted deliberately (an account form reports a malformed address ~500ms later, and its email errors moved from the generic copy to the field-named copy). Component-level gaps stand as recorded.

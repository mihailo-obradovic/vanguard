...
Where it does not, the project owns a small presenter component with a **fixed minimum height** — a message that appears and disappears without one shifts every field below it while the user is mid-form. On `shadcn-vue` the vendored `ui/field` `FieldError` carries that height as an annotated deviation, since upstream unmounts it while the field is valid (`web/CLAUDE.md`, Local invariants); a choice whose inputs reserve the line already solves it.

...
- SPA: `web/pages/users.vue` (the page, its `useMutationDialog` sessions and the delete confirmation), `web/components/users/UsersTable.vue`, `UserFormDialog.vue` (the create/edit form, its rules and the payload it emits) and `UserDetailsDialog.vue`, `web/services/user.api.ts`, `web/services/queries/useUserQueries.ts`, `web/layouts/Default.vue` (nav gating).

...
- `components/_shared/` — auto-imported shared components (`components.dirs` in `nuxt.config.ts`); everything else is explicitly imported. `UIField.vue` and `UIDialog.vue` (+ `UIDialogActions.vue`) remain the primitives `UserFormDialog`/`UserGqlFormDialog`/`ProfileFormDialog` build from, pending their own Phase 4 conversion; `FormDialog.vue` is the shadcn-vue equivalent the auth dialogs use (`web/components/ui/dialog`, `useConfirmOnEnter`, `UIScrollArea` for the scrollable body) — two dialog primitives coexist on this branch until Phase 4 finishes (`catalyst/decisions/014_ui_shadcn-dialog-auth-composition.md`). `AuthCard.vue` is the shell the remaining standalone auth screen (`password-reset/[token]`) is made of, and `RoleBadge.vue` / `VerificationBadge.vue` are the two status pills the table and the profile view share.
- `components/<resource>/` — feature components, imported explicitly: `users/UserFormDialog.vue`, `users/UserGqlFormDialog.vue` and `profile/ProfileFormDialog.vue` own the forms their pages used to hold inline; `users/{Login,Register,ForgotPassword}Dialog.vue` are opened from `web/layouts/Default.vue`, not from a page.
...
- `pages/` — file-based routes: `index`, `home`, `profile`, `users`, `graphql-demo`, plus the one remaining auth page, `password-reset/[token]` (login/register/forgot-password are dialogs now, not routes — `components/users/`).

...
The project was built with [pnpm](https://pnpm.io/) as the primary package manager choice, but it can also work with other options. In that case, the `preinstall` hook in `package.json` should be removed and the `.prettierignore` file should be updated.
...
To facilitate setups of various useful libraries, Nuxt modules were installed. The list includes: ESLint, Image, Test Utils, i18n, Pinia, VueUse, DayJS.

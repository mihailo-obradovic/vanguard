...
5. In separate sessions, each session runs the same independence check itself: list unmerged `feature/*` and `decision/*` branches, read their documents from `master`, compare entry points and protected areas; on overlap, stop and tell the user — do not race the other session.

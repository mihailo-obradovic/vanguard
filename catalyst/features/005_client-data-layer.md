...
- Remaining frontend gaps are outside this layer — pages and logic-bearing components (`decisions/013`).
...
Every Examples row has a spec — the Zod-parse guard, the 419 retry, `placeholderData`, and `['users','fetch']` invalidation on create — each mutation-checked (the source broken deliberately) to confirm the spec fails without it. Frontend suite green at 161 tests; this layer at 100% statements and lines.

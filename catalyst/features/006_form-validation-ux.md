...
Traced against source: the shared factories each form adopts, and the `error-messages` binding on each field; the stricter-client asymmetry cross-checked against feature 003. Frontend suite green at its last recorded run, including the per-form component specs.

Walked live against real MySQL: the account forms render field-named copy inline for an invalid, a taken, and a mixed-case address — with the confirmation disabled while the debounced availability check is in flight — while `/login` accepts a mixed-case address (the read path carries neither rule), a locale switch re-renders open errors, and `ignore_id` goes over the wire (own id from the profile card, the edited user's from the user dialog), so a name-only edit saves with its address untouched. Component-level gaps stand as recorded.

...
Frontend mutation audit (ADR 009; `stryker.config.json`, same cadence and triage rule as the backend's). Scope is the data layer — `web/utils`, `web/services`, `web/stores`, `web/composables` — plus `web/plugins`, `web/regle-config.ts`, and `web/i18n/i18n.config.ts`, which joined once they had tests, and `web/components`, `web/layouts` and `web/app.vue`, which joined once the dialogs, the table and the profile card were tested. Pages and middleware are still out: pages stay on the live browser walk (`decisions/008`), so every mutant there would report as noise rather than signal. Three components are excluded by name — `LoginDialog`, `RegisterDialog`, `ForgotPasswordDialog` — for the same reason; they are this branch's equivalent of master's auth pages and have no specs yet. That exclusion lifts the day they do. The untested composables stay in scope on purpose: Stryker separates `NoCoverage` from `Survived` and prints a covered-code score beside the total, which is the honest analogue of the backend's `--covered-only` without a hand-maintained exclusion list that rots.

Two things about `.vue` in particular:

- **Stryker mutates only the `<script>` block.** Template logic is never mutated, so a component whose branching lives in the template — `UsersTable`'s `v-if="deletable && user.id !== currentUserId"`, for instance — scores on its script alone and can read as 0% while being thoroughly tested. Read a `.vue` score as a statement about its script, not about the component.
- **Compiler macros must be excluded from instrumentation.** Stryker declares its coverage helpers locally inside the script block, and `withDefaults(defineProps<…>(), { … })` and `defineModel(…, { … })` are hoisted out of `setup()` — referencing the helpers from there is a `@vue/compiler-sfc` compile error, not a warning, and it fails the whole dry run with a message that names only the first file it hit. Every affected component therefore carries a `// Stryker disable all` / `// Stryker restore all` pair around its macro block; the prop defaults and model options inside go unmutated as a result. Fourteen components need it on this branch. Master needs none — its components predate the Vuetify rewrite and use neither macro form, carrying only a type-only `defineProps` that has nothing to instrument.
...
- **`layouts/Default.vue` (24 surviving, 19 uncovered — the largest block by far).** Entirely the dialog plumbing (`handleOpen*Dialog`, the three `onSuccess` handlers that close their dialog) and the theme toggle, which the table below already records as the untested remainder. The audit turns that prose into a number and hands the next task a target; nothing here is a false negative in an existing test.
...
| `components/users/{Login,Register,ForgotPassword}Dialog.vue`                    | ~35%  | **No tests yet.** Partially covered incidentally by the layout spec, which renders them. This branch's equivalent of master's auth pages; they stay on the browser walk, and are excluded from the mutate globs by name until they have specs.                                                               |
| `components/_shared/FormCard.vue`                                               | 100%  | **Tested.** The four-branch Enter contract (`confirmOnEnter`, disabled, loading, and the interactive-target exemption) and the slot fallbacks an owner silently replaces.                                                                                                                                    |
| `components/_shared/FullScreenDialog.vue`                                       | 100%  | **Tested.** Same Enter contract with no `confirmOnEnter` opt-out, so the two guards are all that stand between a keypress and a save.                                                                                                                                                                        |
| `components/_shared/CardDialog.vue`                                             | 100%  | **Tested.** Mostly forwarding; the cases that earn their place are the card's own title and buttons surviving when no slots are supplied, and a supplied slot replacing them.                                                                                                                                |
| `components/_shared/PasswordField.vue`                                          | 100%  | **Tested.** The shared `visible` model — a password and its confirmation reveal together, which no assertion on a single field would catch.                                                                                                                                                                  |
| `components/users/UserCard.vue`                                                 | 100%  | **Tested.** The densest component here: edit mode, Escape cancelling from anywhere, and the payload that omits the password pair unless one is being set (the backend reads a present password as a change request).                                                                                         |
| `components/users/UserFormDialog.vue`                                           | 100%  | **Tested.** Reset-on-open in both directions, the password rules switching with the mode, the disabled confirmation that refuses an incomplete form, and a 422 landing on its field.                                                                                                                         |
| `components/users/UserDetailsDialog.vue`                                        | 100%  | **Tested.** Opening on the next subject rather than the last, the emitted payload, and the role select.                                                                                                                                                                                                      |
| `components/users/UsersTable.vue`                                               | 100%  | **Tested.** The row policy: no delete on the signed-in user's own row, none at all for a transport without one, and progress on the row being deleted. Its script scores 0% in the mutation audit because all of that lives in the template.                                                                 |
| `components/_shared/{ConfirmDialog,GapContainer,LinkButton,LoadingSpinner}.vue` | 0-33% | **No tests, deliberately.** Configuration of the dialog base, a type switch, a styled button, and one `calc()` branch — everything assertable about them is a CSS class or a computed style string, which `testing.md` rules out.                                                                            |
...
Four component behaviours are **not observable at this level** and stay on the live browser walk, each recorded where a reader would look for it — in the spec that would otherwise be expected to cover it:

- **`after-leave`** (`CardDialog`, `UserDetailsDialog`) — `v-dialog` fades through `VDialogTransition`, whose leave hook measures the activator and drives the fade with the Web Animations API. happy-dom has no `Element.prototype.animate` and no layout, so the hook never reaches its `done()` callback: the dialog closes, silently. This is the contract that lets a dialog hold its subject's name in the title without it blanking mid-fade.
...
One divergence worth keeping in view: `handleEnterKey` exists in three near-identical copies. `FormCard` and `FullScreenDialog` guard on `closest('button, a, textarea, .v-select')`; `UserCard` guards on `'button, a'` alone. Removing `UserCard`'s guard changes nothing observable — the Vuetify button already swallows the key, and the card renders neither a textarea nor a select — so it was left as it is rather than widened to match. The guards in the other two are load-bearing: they cover plain slot content, and removing either turns their specs red.

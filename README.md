...
Installed Nuxt modules: Fonts, Image, Test Utils, Pinia, Pinia Colada, VueUse. Dates are handled with `temporal-polyfill`, toasts with `vue-sonner`. A single global middleware (`web/middleware/auth.global.ts`) owns all auth routing; the `auth-loader` plugin rehydrates the session at boot. Shared components in `web/components/_shared` are auto-imported.

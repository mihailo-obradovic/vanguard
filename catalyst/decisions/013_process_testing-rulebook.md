...
- **One rulebook.** This record is the single _why_ behind the testing doctrine. The binding rules stay where agents load them: `conventions/testing.md` (universal quality rules) and the stack testing modules (`stacks/backend/laravel/testing.md`, `stacks/frontend/nuxt/testing.md`), which act as the BE/FE annexes. Rejected: separate FE/BE annex files — the stack modules already play that role.

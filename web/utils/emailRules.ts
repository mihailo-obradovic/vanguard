...
// * Mirrors the backend's `lowercase` rule, which only the endpoints that *write* a user carry
// * (register, users, profile). Login and the reset pair deliberately omit it: the column collates
// * case-insensitively, so a mixed-case address still signs in.
...
// ! Fails open on purpose. The endpoint is a convenience; the backend's `unique` rule is the
// ! authority, so an unreachable or rate-limited check must never block a user who would succeed.
const emailAvailable = createRule({
  async validator(value: Maybe<string>, ignoreId?: number) {
    // * Nothing to ask about until the value is a syntactically valid address — `required` and
    // * `email` report those cases, and asking anyway would spend a request on a certain 422.
...
// * The email a user types to identify themselves — login, forgot-password, reset. Matches those
...
// * The email a user claims — register, admin create/edit, profile. Mirrors the write endpoints,
// * including the availability hint. `ignoreId` is the user being edited, where there is one.

...
| `@lucide/vue`                      | frontend       | `variant/shadcn-vue`: the icon set, chosen to match what shadcn-vue's own components import by default — anything else means patching every vendored component and fighting the periodic upstream check (`operations.md`, Vendored shadcn-vue components). Answers the project's open "which icon library" question for this variant specifically.                                                                                                                                                       | user, 2026-09-09 |
...
| `vue-sonner`                       | frontend       | `variant/shadcn-vue`: the toast library behind the `$toast` seam, installed by `shadcn-vue add sonner` and styled through the vendored `ui/sonner` wrapper so toasts take the theme's variables. Replaces `vue-toastification`, whose Vue 3 line (`2.0.0-rc.5`) was published 2021-10-18, never promoted off npm's `next` tag, and has had no release since. Master carries plain `vue-sonner` too, so the `$toast` seam and its spec are identical across branches; only the mounted `Toaster` differs. | user, 2026-09-10 |

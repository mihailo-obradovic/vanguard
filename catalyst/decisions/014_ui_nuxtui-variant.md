...
Mutation run 2026-08-21, after the survivor triage: **76.61% total, 88.86% covered** over 885 scored mutants — 678 killed, 85 survived, 122 without coverage, plus 51 ignored inside the compiler-macro disable pairs. The triage added fourteen tests across the dialogs, `UFormField` and `emailRules`, each verified by breaking its target in the source; the survivors it accepted are recorded with their evidence in `operations.md`. The no-coverage rise over the pre-triage run was unexplained at the time; it is resolved and recorded there.

Mutation run 2026-08-24: **84.86% total, 88.25% covered** — 751 killed, 100 survived, 34 without coverage. Nothing was tested to move it. `@nuxt/test-utils`'s `setupNuxt()` hook was exceeding vitest's 10s `hookTimeout` default against a cold Stryker sandbox, so vitest skipped `UserFormDialog.spec.ts` entirely while still reporting its 17 tests, and all 88 of the file's mutants read `NoCoverage`. `hookTimeout: 60_000` in `vitest.config.ts` fixes it. The covered score slipped 0.61 because the file's 15 real survivors now count, which is the honest direction.

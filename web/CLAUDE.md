...
- `composables/` — `useAppQuery` / `useAppMutation` (the only query/mutation wrappers components may use), `useValidationErrors` / `useExternalErrors` (server-422-to-Regle bridge), `useDialogForm` (a dialog form's cancel/confirm/reset trio, and where the reset-timing rule is decided), `useConfirmOnEnter` (the Enter-confirms handler, and where the list of elements that keep Enter to themselves lives), `useMutationDialog` (a dialog whose confirm runs a mutation — one, or one per mode for a form that both creates and updates: owns the open flag, the subject the dialog is about, the close on success, and keeping 422s off the toast), `useRoleItems` (the two roles as localized select items), `useCookieConsent`.
...
- A list distinguishes its two loading states: `isPending` is the first load and gets the skeleton, `isLoading && !isPending` is a background refetch and gets the refresh bar over rows that stay mounted. The skeleton runs enough placeholder rows to fill the viewport, with the table's scrolling wrapper clipped while they show — a count that stops short of the data replacing it makes the swap jump, and an unclipped surplus flashes a scrollbar that vanishes on arrival. Gating the list itself on `isLoading` tears it down on every invalidation and defeats the `placeholderData` the query wrapper sets (`catalyst/stacks/frontend/nuxt/data-layer.md`).

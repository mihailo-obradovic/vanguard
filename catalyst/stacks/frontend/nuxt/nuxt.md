...
- `@vueuse/core`.
- Vitest, `@nuxt/test-utils`, `@vue/test-utils`, `@vitest/coverage-v8`; `msw` (installed, mocking the API at the wire) and `@testing-library/vue` (installed for the component-test doctrine in `testing.md`, once the first component earned a mounted test).
- `@stryker-mutator/core` + `@stryker-mutator/vitest-runner` — the mutation audit `decisions/013` prescribed, installed once the data-layer tests it was gated on existed (Dependency Change Rule). Dev-only, run by hand, never in CI.

...
The locked invariant is in the Flow Index (`prime-directive.md`). Incident response splits from fixing: stabilize first, fix second.
...
2. **Then fix by the normal rules.** After stabilization the Bug Fixes rule (`prime-directive.md`) applies in full: root cause with evidence, tests, documentation — same change.
...
4. **Every incident leaves a trail:** a decision record of type `incident` (≤60 lines) — what broke, when, what was done (rollback / switch-off / approved hotfix), and what debt remains.
